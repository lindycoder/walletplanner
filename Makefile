...
pydependencies-image: ## Make a Python docker image with pip tools
	docker build --rm --target pydependencies -t $(IMAGE):pydependencies .

jsdependencies-image: ## Make a Node docker image
	docker build --rm --target jsdependencies -t $(IMAGE):jsdependencies .
...
JSDEPENDENCIES_CONTAINER:=docker run -it --rm \
	-v `pwd`/package-lock.json:/usr/src/app/package-lock.json $(IMAGE):jsdev
jsdependencies-update: jsdev-image ## Update package lock
...
PYDEPENDENCIES_CONTAINER:=docker run -it --rm -v `pwd`:/usr/src/app -w /usr/src/app $(IMAGE):pydependencies
pydependencies-update: pydependencies-image ## Update requirements.txt and requirements-test.txt based on the *.in files
	$(PYDEPENDENCIES_CONTAINER) pip-compile --upgrade --no-index --no-emit-trusted-host --output-file requirements.txt requirements.in
	$(PYDEPENDENCIES_CONTAINER) pip-compile --upgrade --no-index --no-emit-trusted-host --output-file requirements-test.txt requirements.txt requirements-test.in

dependencies-update: pydependencies-update jsdependencies-update ## Update dependencies to latests
